...
**Understanding:** Find the maximum heart rate value from all available heart rate data in the dataset, which could come from either the garmin_hr or garmin_stress tables

| Step | Action | Source |
|------|--------|--------|
| 1 | SELECT heart_rate *from* garmin_hr dataset to retrieve all heart rate measurements | garmin_hr |
| 2 | SELECT heart_rate *from* garmin_stress dataset to retrieve all heart rate measurements from stress data | garmin_stress |
| 3 | COMPUTE MAX(heart_rate) *from* the results of both datasets to find the highest heart rate value | combined |
...
**Sensemaker Request:** Find the maximum heart rate value from both garmin_hr and garmin_stress tables by comparing their respective maximum values
