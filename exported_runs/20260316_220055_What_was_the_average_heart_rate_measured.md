...
**Understanding:** Calculate the average heart_rate from garmin_stress table where uid='test004' and status='VALID'

| Step | Action | Source |
|------|--------|--------|
| 1 | Filter garmin_stress table where uid='test004' and status='VALID' | garmin_stress |
| 2 | Calculate aggregate AVG(heart_rate) from filtered rows | step_1_result |
...
**Sensemaker Request:** Calculate AVG(heart_rate) from step_1_result
...
```
54.485022618902065 (Average heart_rate from garmin_stress table filtered by uid='test004' and status='VALID', 1165264 records).
```
