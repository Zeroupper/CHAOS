...
**Understanding:** The query requires analyzing heart rate data from the garmin_stress dataset for user test004, filtering for valid records, and computing the difference between maximum and minimum heart rate values.

| Step | Action | Source |
|------|--------|--------|
| 1 | Filter garmin_stress table for uid = 'test004' and status = 'VALID' and heart_rate is not null | garmin_stress |
| 2 | Calculate the maximum heart_rate value from step 1 result | step_1_result |
| 3 | Calculate the minimum heart_rate value from step 1 result | step_1_result |
| 4 | Calculate the difference between max heart_rate (step 2) and min heart_rate (step 3) | step_2_result, step_3_result |

## Execution Log


**Sensemaker Request:** SELECT * FROM garmin_stress WHERE uid = 'test004' AND status = 'VALID' AND heart_rate IS NOT NULL
...
**Sensemaker Request:** SELECT MAX(heart_rate) AS max_heart_rate FROM (SELECT * FROM garmin_stress WHERE uid = 'test004' AND status = 'VALID' AND heart_rate IS NOT NULL) AS step_1_result
...
**Sensemaker Request:** SELECT MIN(heart_rate) AS min_heart_rate FROM (SELECT * FROM garmin_stress WHERE uid = 'test004' AND status = 'VALID' AND heart_rate IS NOT NULL) AS step_1_result
...
```
100.0 bpm (maximum: 100.0, minimum: 0.0 from 1165264 valid records filtered for user test004)
```
