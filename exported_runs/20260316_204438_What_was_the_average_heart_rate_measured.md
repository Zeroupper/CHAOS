...
**Understanding:** This query requires calculating the average heart_rate from the garmin_stress dataset, filtering for user 'test004' and status 'VALID'

| Step | Action | Source |
|------|--------|--------|
| 1 | Filter garmin_stress dataset where uid = 'test004' AND status = 'VALID' to get valid stress sensor readings | garmin_stress |
| 2 | Calculate the average of heart_rate column from the filtered results | step_1_result |
...
**Sensemaker Request:** Calculate the average of heart_rate column from step_1_result (filtered garmin_stress data where uid = 'test004' AND status = 'VALID')
...
```
54.49 (average heart rate from filtered garmin_stress dataset for user test004 with VALID status)
```
