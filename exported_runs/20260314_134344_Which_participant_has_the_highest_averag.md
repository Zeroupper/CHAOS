...
**Understanding:** Need to find the participant with the highest average 14-day rolling intraday step count from the steps dataset, then look up their end-of-term BDI-II depression score from the dep_endterm dataset. If no score exists, return -1.

| Step | Action | Source |
|------|--------|--------|
| 1 | Load the steps dataset and select columns: pid, f_steps:fitbit_steps_intraday_rapids_sumsteps:14dhist | steps |
| 2 | Group by pid and calculate the average of f_steps:fitbit_steps_intraday_rapids_sumsteps:14dhist for each participant | step_1_result |
| 3 | Sort by average step count descending and select the top participant (highest average) | step_2_result |
| 4 | Load the dep_endterm dataset and select columns: pid, BDI2 | dep_endterm |
| 5 | Left join step_3_result with step_4_result on pid, select BDI2 column, replace null values with -1 | step_3_result, step_4_result |

## Execution Log


**Sensemaker Request:** Load the steps dataset and select columns: pid, f_steps:fitbit_steps_intraday_rapids_sumsteps:14dhist
...
**Sensemaker Request:** Group by pid and calculate the average of f_steps:fitbit_steps_intraday_rapids_sumsteps:14dhist for each participant
...
**Sensemaker Request:** Sort by average step count descending and select the top participant (highest average)
...
**Result:**
```
{"pid":{"99":"INS-W_126"},"f_steps:fitbit_steps_intraday_rapids_sumsteps:14dhist":{"99":225983.2307692308}}
```


**Sensemaker Request:** Load the dep_endterm dataset and select columns: pid, BDI2
...
**Sensemaker Request:** Left join step_3_result with step_4_result on pid, select BDI2 column, replace null values with -1
...
```
10.0 (BDI-II score for participant INS-W_126 with highest average 14-day rolling step count)
```
