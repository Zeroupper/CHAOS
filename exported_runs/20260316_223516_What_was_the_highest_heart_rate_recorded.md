...
**Understanding:** The query asks for the maximum/highest heart rate value. The relevant dataset is 'garmin_hr' which contains heart_rate column with float64 values. There's also garmin_stress with heart_rate column but it has many null values, so garmin_hr is the primary source.

| Step | Action | Source |
|------|--------|--------|
| 1 | Load the garmin_hr dataset to access heart rate measurements | garmin_hr |
| 2 | Calculate the maximum value from the heart_rate column to find the highest recorded heart rate | garmin_hr.heart_rate |
...
**Sensemaker Request:** Calculate the maximum value from the heart_rate column in garmin_hr to find the highest recorded heart rate
