...
**Understanding:** Extract average heart rate from garmin_stress dataset for user test004 where status is 'VALID'

| Step | Action | Source |
|------|--------|--------|
| 1 | Filter garmin_stress table for uid = 'test004' AND status = 'VALID' | garmin_stress |
| 2 | Calculate average of heart_rate column from filtered data | step_1_result |
...
**Sensemaker Request:** Calculate average of heart_rate column from filtered data (step_1_result)
...
```
54.49 bpm (average heart rate from 1165264 records for user test004 with VALID status)
```
