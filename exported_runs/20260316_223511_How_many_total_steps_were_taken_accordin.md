...
**Understanding:** The user wants to find the sum/total of steps recorded by the Garmin wearable device. This requires accessing the garmin_steps dataset and aggregating the steps data.

| Step | Action | Source |
|------|--------|--------|
| 1 | Load garmin_steps dataset from storage | garmin_steps |
| 2 | Filter out any null values from total_steps column | step_1_result |
| 3 | Sum all values in total_steps column to get total steps count | step_2_result |

## Execution Log


**Sensemaker Request:** Load the garmin_steps dataset from storage using the garmin_steps table
...
**Sensemaker Request:** Filter out any null values from total_steps column in the garmin_steps dataset
...
**Sensemaker Request:** Sum all values in total_steps column from step_2_result to get total steps count
...
```
9852389.0 total steps (aggregated sum from the garmin_steps dataset after removing null values).
```
