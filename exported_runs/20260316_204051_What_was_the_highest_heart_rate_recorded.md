...
**Understanding:** Find the maximum heart rate value across all datasets that contain heart rate measurements.

| Step | Action | Source |
|------|--------|--------|
| 1 | Select and filter heart_rate values from garmin_hr dataset | garmin_hr |
| 2 | Select and filter heart_rate values from garmin_stress dataset | garmin_stress |
| 3 | Apply MAX() aggregation to find highest heart rate from combined data | combined heart rate data from step_1_result and step_2_result |
...
**Sensemaker Request:** SELECT heart_rate FROM garmin_stress
...
**Sensemaker Request:** combined_max = max(step_1_result['heart_rate'].values(), step_2_result.values())
...
**Result:**
```
Maximum heart rate from combined data: 155.0
```
...
```
155.0 (Maximum heart rate found by combining valid measurements from both garmin_hr and garmin_stress datasets).
```
