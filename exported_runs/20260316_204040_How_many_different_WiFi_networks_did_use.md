...
**Understanding:** Count the number of unique WiFi networks (by bssid) that user test004 connected to their device.

| Step | Action | Source |
|------|--------|--------|
| 1 | Filter the ios_wifi table to select records where uid = 'test004' | ios_wifi |
| 2 | Select unique bssid values from the filtered results (each unique bssid represents a different WiFi network) | - |
| 3 | Count the total number of unique WiFi networks (bssid values) | - |

## Execution Log


**Sensemaker Request:** Filter the ios_wifi table to select records where uid = 'test004'
...
**Sensemaker Request:** Select unique bssid values from the filtered results (excluding null values), then count the total number of unique WiFi networks
...
```
16 (unique non-null BSSIDs for user test004)
```
